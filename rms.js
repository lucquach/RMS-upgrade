...
app.post("/", async function(req, res){
  ...
  try
  {
    ...
    // Add destroyed handler — fires when GumJS destroys the script (e.g. session
    // detaches). Null out api/script here to prevent "Script is destroyed" errors
    // from any in-flight route handlers that still hold a reference.
    script.destroyed.connect(() => {
      console.log('[*] Script destroyed — clearing api reference');
      api = null;
    });

    //loading FRIDA startup script BEFORE resume so hooks are active
    //before Application.onCreate / ContentProvider.attachInfo run.
    //Anti-Frida checks (e.g. VNPAY DP:751) fire very early in app init.
    if (frida_script)
      await api.loadcustomfridascript(frida_script)

    //loading APIs Monitors BEFORE resume for the same reason
    if(api_selected)
    {
      //load API Monitors list
      const api_monitor = read_json_file(API_MONITOR_FILE_PATH)
    ...
    // Resume AFTER all hooks are installed — critical for early init detection bypass
    if (mode == "Spawn" && target_package!="Gadget")
      device.resume(pid)
...
function onProcessCrashed(crash) {
  ...
  // Null out api so route guards (if (!api)) block stale RPC calls
  // on the now-destroyed script and prevent "Script is destroyed" errors.
  api = null;
  pid = null;
}
...
function onSessionDetached(reason, crash) {
  ...
  // Reset api on any detach (crash, killed, or normal exit).
  // All File Manager / Dump / etc. routes check `if (!api)` first,
  // so this makes them return a clean 503 instead of throwing.
  api = null;
  pid = null;
}
...
app.get("/static_analysis", async function(req, res){
  if (!api) return res.redirect('/?frida_crash=True&frida_crash_message=No+active+Frida+session.+Please+attach+to+an+app+first.');
...
/*
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
File Manager - TAB (Full Feature: List / Read / Download / Edit / Delete / SQLite)
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
*/
...
// ── GET /file_manager — list directory
app.get("/file_manager", async function(req, res){

  // Guard: session must be attached
  if (!api) {
    return res.render("file_manager.html", {
      mobile_OS, target_package, system_package, no_system_package,
      env: {}, files_at_path: "", currentPath: undefined,
      error: "No active Frida session. Please attach to an app first."
    });
  }
  ...
  // Load app env info once per session
  if (Object.keys(app_env_info).length === 0) {
    try {
      app_env_info = await api.getappenvinfo()
    } catch(err) {
      console.log("getappenvinfo error: "+err)
    }
  }

  if(dirPath) {
    try {
      files_at_path = await api.listfilesatpath(dirPath)
    } catch(err) {
      console.log("listfilesatpath error: "+err)
    }
  }
...
// ── GET /file_manager/read — read file content (returns JSON)
app.get("/file_manager/read", async function(req, res){
  if (!api) return res.json({ success: false, error: 'No active Frida session' })
  const filePath = req.query.path
  if (!filePath) return res.json({ success: false, error: 'Missing path parameter' })
  try {
...
// ── GET /file_manager/download — stream binary file to browser
app.get("/file_manager/download", async function(req, res){
  if (!api) return res.status(503).send('No active Frida session')
  const filePath = req.query.path
  if (!filePath) return res.status(400).send('Missing path')
  try {
    const result = await api.readfileatpath(filePath)
    if (!result.success) return res.status(500).send('Error: ' + result.error)
    // Guard against null/undefined content (e.g. permission error returns success=false above)
    if (result.content == null) return res.status(500).send('Error: file content is null')
    const filename = path_module.basename(filePath)
    const buf = Buffer.from(result.content, 'base64')
    // RFC 5987 encoding for filenames with special/non-ASCII characters
    const encodedFilename = encodeURIComponent(filename).replace(/'/g, '%27')
    res.setHeader('Content-Disposition',
      `attachment; filename="${filename.replace(/"/g, '_')}"; filename*=UTF-8''${encodedFilename}`)
...
// ── POST /file_manager/write — overwrite file content (body: {path, content (base64)})
app.post("/file_manager/write", async function(req, res){
  if (!api) return res.json({ success: false, error: 'No active Frida session' })
  const filePath = req.body.path
  const b64content = req.body.content
  if (!filePath || b64content === undefined)
    return res.json({ success: false, error: 'Missing path or content' })
  try {
    const result = await api.writefileatpath(filePath, b64content)
    res.json(result)
  } catch(err) {
    res.json({ success: false, error: err.toString() })
  }
...
// ── POST /file_manager/delete — delete file
app.post("/file_manager/delete", async function(req, res){
  if (!api) return res.json({ success: false, error: 'No active Frida session' })
  const filePath = req.body.path
  if (!filePath) return res.json({ success: false, error: 'Missing path' })
  try {
    const result = await api.deletefileatpath(filePath)
    res.json(result)
  } catch(err) {
    res.json({ success: false, error: err.toString() })
  }
...
// ── GET /file_manager/sqlite_tables — list SQLite tables
app.get("/file_manager/sqlite_tables", async function(req, res){
  if (!api) return res.json({ success: false, error: 'No active Frida session' })
  const filePath = req.query.path
  if (!filePath) return res.json({ success: false, error: 'Missing path' })
  try {
    const result = await api.getsqlitetables(filePath)
    res.json(result)
  } catch(err) {
    res.json({ success: false, error: err.toString() })
  }
...
// ── GET /file_manager/sqlite_query — execute query on SQLite DB
app.get("/file_manager/sqlite_query", async function(req, res){
  if (!api) return res.json({ success: false, error: 'No active Frida session' })
  const filePath = req.query.path
  const query = req.query.query || 'SELECT * FROM sqlite_master'
  if (!filePath) return res.json({ success: false, error: 'Missing path' })
  try {
    const result = await api.querysqlite(filePath, query)
    res.json(result)
  } catch(err) {
    res.json({ success: false, error: err.toString() })
  }
...
app.post("/file_manager", async function(req, res){
  // kept for backwards compat — redirect to GET
  return res.redirect('/file_manager')
})
/*
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
